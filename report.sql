...
with pas_not_valid as (select client_key,passport_num,last_name||'' ''||first_name||'' ''|| patronymic fio, phone 
     ...
     account_cl as     (select passport_num, fio, phone,account_key 
     					from chirkova.DIM_accounts inner join pas_not_valid using(client_key))
     					
	select trans_date,passport_num, fio, phone, ''Совершение операции при просроченном паспорте'' type_fraud, now() from 
            (select trans_date,passport_num, fio, phone,card_key from account_cl cl_ac inner join chirkova.DIM_cards car using(account_key)
            inner join d_trans using (card_key)) q );
					   


insert into chirkova.REPORT (          
with ac_not_valid as       (select account_key, client_key,valid_to from chirkova.DIM_accounts where valid_to<='''||cur_day||'''),
     ...
     account_cl as         (select passport_num, last_name||'' ''||first_name||'' ''|| patronymic fio, phone,account_key
     					   from chirkova.DIM_clients inner join ac_not_valid using(client_key))
...
insert into chirkova.REPORT (  
 with current_hour as       (select card_key, trans_date, terminal_key from(
 						    ...
 						    from  chirkova.FACT_TRANSACTIONS where date(trans_date)='''||cur_day||''') a where delta<''1:00:00'' or delta_2<''1:00:00''),
 	  ...
 	  another_city as 		(select card_key, max(trans_date) trans_date from (
 			                 	select card_key, c_t, max(trans_date) trans_date, sum(c_t) c_c from(select card_key, terminal_city, count(terminal_city) c_t, max(trans_date) trans_date from 
 	  								current_hour inner join chirkova.DIM_terminals using(terminal_key) group by card_key,terminal_city) q1 group by card_key, c_t) q2
      ...
      select account_key,client_key, trans_date from another_city  inner join chirkova.DIM_cards using(card_key) inner join chirkova.DIM_accounts using(account_key)) j inner join
      			chirkova.DIM_clients using(client_key)) ;
...
with delta_time_count as ( select card_key, trans_date, delta,delta_2,c_t, amt,oper_result,
							...
							from(
 									...
 									from  chirkova.FACT_TRANSACTIONS where date(trans_date)='''||cur_day||''') a
      ...
      result_num as  ( select card_key,max(trans_date) trans_date from chek 
 	  ...
 	  select trans_date,passport_num, last_name||'' ''||first_name||'' ''|| patronymic fio, phone, ''Попытка подбора сумм'' type_fraud, now() from(
     	 select client_key,trans_date from (
     		 select account_key, trans_date from result_num  inner join chirkova.DIM_cards using(card_key)) a_n inner join chirkova.DIM_accounts using(account_key)) d inner join
      			chirkova.DIM_clients using(client_key));
